...
void WorldHub::Update(Time localTime_)
{
	ProfileFunction();
...
ActorUID WorldHub::NewActorUID()
{
	return (ActorUID)nextActorUID++;
}

WorldHub::ActorPlayer& WorldHub::SpawnPlayerActor(i32 clientID, ClassType classType, SkinIndex skinIndex, const wchar* name, const wchar* guildTag)
{
...
WorldHub::ActorPlayer& WorldHub::SpawnPlayerSubActor(i32 clientID, ActorUID parentActorUID, ClassType classType, SkinIndex skinIndex)
{
...
WorldHub::ActorNpc& WorldHub::SpawnNpcActor(CreatureIndex docID, i32 localID)
{
	ActorUID actorUID = NewActorUID();
...
WorldHub::ActorJukebox& WorldHub::SpawnJukeboxActor(CreatureIndex docID, i32 localID, const vec3& pos, const vec3& dir)
{
...
WorldHub::ActorPlayer* WorldHub::FindPlayerActor(ActorUID actorUID) const
{
...
WorldHub::ActorNpc* WorldHub::FindNpcActor(ActorUID actorUID) const
{
...
WorldHub::ActorNpc* WorldHub::FindNpcActorByCreatureID(CreatureIndex docID)
{
	foreach(it, actorNpcList) {
		if(it->docID == docID) {
			return &(*it);
...
bool WorldHub::DestroyPlayerActor(ActorUID actorUID)
{
