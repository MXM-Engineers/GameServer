...
- **Encrypted client sends**: some client->server netids (60178, 60179, 60195, 60196, 60197) carry LEA-encrypted payloads on the wire — layouts are verifiable only from builders, never from captures. Their bytes raise `UnicodeDecodeError` (not `struct.error`) once a layout reads a wstr; `wireshark_to_raw.py` catches both. A capture "smoke" passing therefore does NOT prove these layouts; conversely, decode failures on exactly these netids are expected noise.
- **Writer type ambiguity**: the u32-writer helpers (FUN_0056e0db/e086/e130...) also carry float bits — f32 vs u32 must be resolved from caller context or capture plausibility, not from the writer alone (positions/rotations/speeds are floats).
